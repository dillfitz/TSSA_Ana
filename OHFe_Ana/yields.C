void yields()
{
	...
	TFile* in2 = new TFile( "templates_new_n_n_n_n_n_n.root", "READ" );
	...
	for (int i=0; i<nPt_bins; i++) {
		...
		//std::cout << " my error : " << N_df_e[i] << std::endl;
		//std::cout << " Tim's yields : " <<  N_tr[i] << std::endl;
	}
	...
	gr_df->SetTitle("electron yields (PPG223 ana.538)");
	...
	gr_ratio->SetTitle("electron yield ratios (PPG223 ana.538)");
