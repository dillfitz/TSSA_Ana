void hadrondepcheck_minus()
{
  ...
  bool conveto;
  int ndf, hitpat, quality, nhit, n0,triginfo,charge;
  ...
  hSvxTree->SetBranchAddress("charge",&charge);
...
for ( int i = 0; i < hSvxTree->GetEntries();i++)
    {
      ...
      if (nv)
	{
	  if ((quality == 31 || quality == 63) && (hitpat&3) == 3 && nhit > 2 && chisq/ndf <= 3 && fabs(zed) <= 75 && fabs(sigemcdphi) <= 3 && fabs(sigemcdz) <= 3 && disp <= 5 && charge==-1 )
	    {
	...
	{
	  if ((quality == 31 || quality == 63) && (hitpat&3) == 3 && nhit > 2 && chisq/ndf <= 3 && fabs(zed) <= 75 && fabs(sigemcdphi) <= 3 && fabs(sigemcdz) <= 3 && disp <= 5 && conveto == 1 && charge==-1 )
	    {
