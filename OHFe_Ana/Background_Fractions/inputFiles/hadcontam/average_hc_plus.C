...
void average_hc_plus()
{
  const bool nv = 0;
  const int nbins = NUM_VALUE_BINS;

  if (nv)
    TFile* infile1 = TFile::Open("depfitting/dataFiles/hadronnorm_depfit_noveto_plus.root");
  else
    TFile* infile1 = TFile::Open("depfitting/dataFiles/hadronnorm_depfit_plus.root");

  TH1F* h_depfit = (TH1F*)infile1->Get("h_hadfrac")->Clone("h_depfit");

  if (nv)
    TFile*infile2= TFile::Open("algebraic/dataFiles/algebraic_hadroncontam_noveto_plus.root");
  else
    TFile*infile2= TFile::Open("algebraic/dataFiles/algebraic_hadroncontam_plus.root");
  ...
  c1->SaveAs("hadcontammethods_plus.png");
