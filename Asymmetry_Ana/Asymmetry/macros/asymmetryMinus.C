...
void asymmetryMinus( const char* particle = "ohfe",
		...
		const bool isolated = false )//both must be set to false for dp
{
  ...
  outputFileName += "_AN_fills_minus.root";
...
//	  }	  
  ...
  for( int ptBin = 0; ptBin < NUM_VALUE_BINS; ptBin++ )
	...
	{
	  ostringstream title;
	  title << BEAM_NAMES[ beam ] << OPTION_NAMES[o] << " Minus " 
		<< " Asymmetry from p_{T} " << VALUE_BINS[ ptBin ] << " to "
		<< VALUE_BINS[ ptBin + 1 ] << " GeV; Fill Group Index; ";

	  ostringstream name;
	  name << BEAM_NAMES[ beam ] << OPTION_NAMES[o] << "_Minus_" 
