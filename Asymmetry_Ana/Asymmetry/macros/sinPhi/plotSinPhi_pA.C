...
void plotSinPhi_pA( const char* particle = "pi0", const char* collisionSystem = "pAu" , const bool background = false )
{
  ...
  float YMIN[NUM_PT_BINS];
  float YMAX[NUM_PT_BINS];   
    ...
    for( int op = 0; op < NUM_OPTIONS_PHI; op++ )
      for( int ptBin = 0; ptBin < NUM_PT_BINS; ptBin++ )
        ...
        //if (b == YELLOW) {continue;}
	      ostringstream name;
	      name << BEAM_NAMES[ b ] << OPTION_NAMES[ op ] << PT_BINS[ ptBin ] << "to" << PT_BINS[ ptBin + 1 ];
  ...
  for( int b = 0; b < NUM_BEAMS; b++ )
    for( int ptBin = 0; ptBin < NUM_PT_BINS; ptBin++ )
    {
      ...
      if (b==YELLOW) {continue;}
	    ...
	    TH1 *frame = new TH2F( frameName, " ", 1, 0.0, 2*PI, 1, YMIN[ ptBin ], YMAX[ ptBin ] );
	    frame->SetStats(0);
	    ...
	    newAsGraphs[b][ ptBin ] = new TGraphAsymmErrors( NUM_PHI_BINS*2, newPhiCent, newAs, newPhiLow, newPhiHigh, newAsErr, newAsErr );
  ...
  for( int ptBin = 0; ptBin < NUM_PT_BINS; ptBin++ )
  {
   ...
   // weightedAverage( amp[ YELLOW ][ ptBin ], ampErr[ YELLOW ][ ptBin ], amp[ BLUE   ][ ptBin ], ampErr[ BLUE   ][ ptBin ],finalAmp[ ptBin ], finalAmpErr[ ptBin ] );
  }

  asymmetry = amp[ BLUE ];
  asymmetryErr = ampErr[ BLUE ];
  TGraphAsymmErrors *blue = new TGraphAsymmErrors( NUM_PT_BINS, PT_BIN_CENTERS, asymmetry, ptLow, ptHigh, asymmetryErr, asymmetryErr );
  ...
  */
  ...
  otherBlue->SetMarkerStyle( kFullSquare );
  otherBlue->SetTitle("Yellow Blue Asymmetry From Integrating Over Arms; p_{T} [GeV];" );
  otherBlue->GetYaxis()->SetRangeUser( AVE_YMIN, AVE_YMAX );
  otherBlue->Draw( "AP" );
  zeroLine->Draw( "same" );
  //*/
  TCanvas *c52 = new TCanvas( "c52", "the finals" );
  TGraphAsymmErrors *lumi = new TGraphAsymmErrors( NUM_PT_BINS, PT_BIN_CENTERS, amp[BLUE], ptLow, ptHigh, ampErr[BLUE], ampErr[BLUE] );
  ...
  lumi->Draw( "AP" );
  otherBlue->SetMarkerColor( kViolet - 6 );
  otherBlue->SetMarkerStyle( kFullCircle );
  otherBlue->Draw( "P" );
  zeroLine->Draw( "same" );
  ...
  legend->AddEntry( otherBlue, "From Integrating Over Arms", "lep" );
  legend->Draw();
  if( SAVE_PICTURES )
  {
