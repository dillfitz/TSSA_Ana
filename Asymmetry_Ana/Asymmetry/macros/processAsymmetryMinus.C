...
void processAsymmetryMinus( const char* particle = "ohfe",
...
//both must be set to false for dp
{
 ...
 //   cout << "Error! particle can only be dp, pi0, or eta!  You are calculating nonsense " << endl;
  ...
  for( int ptBin = 0; ptBin < NUM_VALUE_BINS; ptBin++ )
	...
	{
	  ostringstream names;
	  names << BEAM_NAMES[ i ] << OPTION_NAMES[j] << "_Minus_" 
  ...
  for( int ptBin = 0; ptBin < NUM_VALUE_BINS; ptBin++ )
    {
      TString name = "c";
      name += ptBin;
      lumiCanvases[ ptBin ] = new TCanvas( name, canvasTitle[ ptBin ], 
					   width, heightLumi );
      lumiCanvases[ ptBin ]->Divide( 2, 2 );
	 ...
	 //lumi left then lumi right
	  {
	    ...
	    drawGraph( graph, ptBin, beam, option, 
		       fitValueInput, fitValueErrInput, lumiChi2Histo );
  ...
  TH1D *sqrtChi2Histo = new TH1D( "sqrtChi2Histo", 
			      "Reduced #chi^{2} Distribution for Sqrt Formula ",
			      20, 0.0, 2 );
  ...
  for( int ptBin = 0; ptBin < NUM_VALUE_BINS; ptBin++ )
    {
      TString name = "c";
      name += 10 + ptBin;
      sqrtCanvases[ ptBin ] = new TCanvas( name, canvasTitle[ ptBin ], 
					   width, heightSqrt );
      ...
      for( int beam = 0; beam < NUM_BEAMS; beam++ )
	{
	  ...
	  drawGraph( graph, ptBin, beam, SQRT, 
		     fitValueInput, fitValueErrInput, sqrtChi2Histo );
  ...
  for( int i = 0; i < NUM_VALUE_BINS; i++ )
    {
      weightedAverage(
	   fitValues[ YELLOW ][ LEFT  ][i], fitValuesErr[ YELLOW ][ LEFT  ][i],
           fitValues[ YELLOW ][ RIGHT ][i], fitValuesErr[ YELLOW ][ RIGHT ][i],
	   lumiY[i], lumiYErr[i] );

      weightedAverage(
	   fitValues[ BLUE ][ LEFT  ][i], fitValuesErr[ BLUE ][ LEFT  ][i],
           fitValues[ BLUE ][ RIGHT ][i], fitValuesErr[ BLUE ][ RIGHT ][i],
	   lumiB[i], lumiBErr[i] );
      ...
      weightedAverage(
	   fitValues[ YELLOW ][ SQRT ][i], fitValuesErr[ YELLOW ][ SQRT ][i],
	   fitValues[ BLUE   ][ SQRT ][i], fitValuesErr[ BLUE   ][ SQRT ][i],
  ...
  graphLumiY->SetName( "lumiY_minus" );
  graphLumiY->SetTitle( "lumiY_minus" );
  graphLumiY->Write( "lumiY_minus" );
  graphLumiY->Delete();

  TGraphAsymmErrors *graphLumiB = 
    new TGraphAsymmErrors( NUM_VALUE_BINS, BIN_CENTERS, lumiB, 
			   ptLow, ptHigh, lumiBErr, lumiBErr );
  graphLumiB->SetName( "lumiB_minus" );
  graphLumiB->SetTitle( "lumiB_minus" );
  graphLumiB->Write( "lumiB_minus" );
  graphLumiB->Delete();
  ...
  graphLumi->SetName( "lumi_minus" );
  graphLumi->SetTitle( "lumi_minus" );
  graphLumi->Write( "lumi_minus" );
  graphLumi->Delete();
  ...
  graphSqrt->SetName( "sqrt_minus" );
  graphSqrt->SetTitle( "sqrt_minus" );
  graphSqrt->Write( "sqrt_minus" );
  graphSqrt->Delete();
