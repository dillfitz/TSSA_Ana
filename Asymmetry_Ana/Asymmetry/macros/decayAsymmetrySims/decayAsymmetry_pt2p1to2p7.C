...
void decayAsymmetry_pt2p1to2p7()
{
  gStyle->SetOptStat(0);
  gStyle->SetOptFit();
  ...
  TFile *outFile = new TFile("dataFiles/jpsiDecayAsymmetry_ptbin2p1to2p7.root", "RECREATE");
  ...
  decayTree->Draw("phi>>jphi_presel","(pt1>2.1&&pt1<2.7)||(pt2>2.1&&pt2<2.7)");
  ...
  for (int i=0; i<nAN; ++i)
    {
      histname = "jpsi_phi_pt3_AN_" + AN_labels[i];
      jphis[i] = new TH1F(histname, ";#phi;" ,6,0,2*PI);
      jphis[i]->Sumw2();

      histname = "ep_phi_pt3_AN_" +  AN_labels[i];
      epphis[i] = new TH1F(histname,";#phi;",6,0,2*PI);
      epphis[i]->Sumw2();

      histname = "em_phi_pt3_AN_" +  AN_labels[i];
      emphis[i] = new TH1F(histname,";#phi;",6,0,2*PI);
      emphis[i]->Sumw2();

      histname = "e_phi_pt3_AN_" +  AN_labels[i];
  ...
  for (int i=0; i<nEntries; ++i)
    {
      ...
      if ((pt1 > 2.1 &&  pt1 < 2.7) || ( pt2 > 2.1 && pt2 < 2.7)) 
	{
	  jpt->Fill(pt);
  ...
  nomx.Draw("SAME");
  nomy.Draw("SAME");
  c1->SaveAs("images/dilutions_ptbin2p1to2p7.png");

  TCanvas *c2 = new TCanvas("c2");
  jphis[5]->Draw();
  c2->SaveAs("images/jphiAN_pt2p1to2p7.png");
  
  TCanvas *c3 = new TCanvas("c3");
  ephis[5]->Draw();
  c3->SaveAs("images/ephiAN_pt2p1to2p7.png");

  TCanvas *c4 = new TCanvas("c4");
  ejpsiphi->Draw("COLZ");
  c4->SaveAs("images/ejpsiphi_pt2p1to2p7.png");
