...
void decayAsymmetry_pt1to1p3()
{
  ...
  TFile *outFile = new TFile("dataFiles/jpsiDecayAsymmetry_ptbin1to1p3.root", "RECREATE");
  ...
  decayTree->Draw("phi>>jphi_presel","(pt1>1.0&&pt1<1.3)||(pt2>1.0&&pt2<1.3)");
  ...
  for (int j=1; j<=6; ++j)
  {
	phiCounts.push_back(jphi_presel->GetBinContent(j));
  }

  int phiMin = *min_element(phiCounts.begin(), phiCounts.end());
  cout << "phi Min : " << phiMin << " phi Fit : " << phiHeight->GetParameter(0) << endl;
  TString histname = "";
  for (int i=0; i<nAN; ++i)
    {
      histname = "jpsi_phi_pt1to1p3_AN_" + AN_labels[i];
      jphis[i] = new TH1F(histname, ";#phi;" ,6,0,2*PI);
      jphis[i]->Sumw2();

      histname = "ep_phi_pt1to1p3_AN_" +  AN_labels[i];
      epphis[i] = new TH1F(histname,";#phi;",6,0,2*PI);
      epphis[i]->Sumw2();

      histname = "em_phi_pt1to1p3_AN_" +  AN_labels[i];
      emphis[i] = new TH1F(histname,";#phi;",6,0,2*PI);
      emphis[i]->Sumw2();

      histname = "e_phi_pt1to1p3_AN_" +  AN_labels[i];
...
*/
  ...
  for (int i=0; i<nEntries; ++i)
    {
      ...
      if ((pt1>1.0 && pt1 <1.3) || (pt2>1.0 && pt2 < 1.3))
	{
	  jpt->Fill(pt);
  ...
  nomx.Draw("SAME");
  nomy.Draw("SAME");
  c1->SaveAs("images/dilutions_ptbin1to1p3.png");

  TCanvas *c2 = new TCanvas("c2");
  jphis[5]->Draw();
  c2->SaveAs("images/jphiAN_pt1to1p3.png");
  
  TCanvas *c3 = new TCanvas("c3");
  ephis[5]->Draw();
  c3->SaveAs("images/ephiAN_pt1to1p3.png");

  TCanvas *c4 = new TCanvas("c4");
  ejpsiphi->Draw("COLZ");
  c4->SaveAs("images/ejpsiphi_pt1to1p3.png");
