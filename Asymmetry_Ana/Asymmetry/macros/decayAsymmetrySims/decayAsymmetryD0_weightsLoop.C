...
bool verbosity = 1;
void decayAsymmetryD0_weightsLoop()
{
  gStyle->SetOptFit();
    ...
    for (int k=0; k<Npar; ++k)
	  {
        ...
        //cout << " j : " << j << " k : " << k << " d0 pT bin : " << i << " Asymmetry value : " << AN[j][k][i] << endl;	      
	    } 
	  }
  }
  float Pol = 1;
  ...
  TFile *outFile = new TFile("dataFiles/d0DecayAsymmetry_weightsLoop.root", "RECREATE");
    ...
    for (int l=0; l<Npar; ++l)
	  {
	    for(int m=0; m<Npar; ++m)
	    {
	      ...
	      for (int j=0; j<nd0ptbins; ++j)
		    ...
		    //cout << " l : " << l << " m : " << m << " d0 pT bin : " << d0PtIndex << " Asymmetry value : " << AN[l][m][d0PtIndex] << endl;
