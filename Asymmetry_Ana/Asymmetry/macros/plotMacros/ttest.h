...
TGraph *ttest( TGraphAsymmErrors *graph1, TGraphAsymmErrors* graph2,
	       int correlation )//either 0 or 1
{
  ...
  if( correlation == 0 )
    ...
    cout << "Error!  TTest function only designed to work for data sets that"
	 << " are either 100% or 0% correlated" << endl;
  ...
  for( int i = 0; i < numPoints1; i++ )
    {
      ...
      ttestValues[i] /= sqrt( fabs( asymmetryErr1[i]*asymmetryErr1[i]
				   + sign*asymmetryErr2[i]*asymmetryErr2[i] ) );
