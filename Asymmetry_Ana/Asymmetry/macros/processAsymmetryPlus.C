...
void processAsymmetryPlus( const char* particle = "ohfe",
...
//both must be set to false for dp
{
 ...
 //   cout << "Error! particle can only be dp, pi0, or eta!  You are calculating nonsense " << endl;
  ...
  for( int ptBin = 0; ptBin < NUM_VALUE_BINS; ptBin++ )
	...
	{
	  ostringstream names;
	  names << BEAM_NAMES[ i ] << OPTION_NAMES[j] << "_Plus_" 
  ...
  graphLumiY->SetName( "lumiY_plus" );
  graphLumiY->SetTitle( "lumiY_plus" );
  graphLumiY->Write( "lumiY_plus" );
  graphLumiY->Delete();

  TGraphAsymmErrors *graphLumiB = 
    new TGraphAsymmErrors( NUM_VALUE_BINS, BIN_CENTERS, lumiB, 
			   ptLow, ptHigh, lumiBErr, lumiBErr );
  graphLumiB->SetName( "lumiB_plus" );
  graphLumiB->SetTitle( "lumiB_plus" );
  graphLumiB->Write( "lumiB_plus" );
  graphLumiB->Delete();
  ...
  graphLumi->SetName( "lumi_plus" );
  graphLumi->SetTitle( "lumi_plus" );
  graphLumi->Write( "lumi_plus" );
  graphLumi->Delete();
  ...
  graphSqrt->SetName( "sqrt_plus" );
  graphSqrt->SetTitle( "sqrt_plus" );
  graphSqrt->Write( "sqrt_plus" );
  graphSqrt->Delete();
