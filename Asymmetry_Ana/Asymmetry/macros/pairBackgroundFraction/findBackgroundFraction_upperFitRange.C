...
{
  { 60000, 0.548, 0.03, 10000,   10000,   -10000,  -10000 },//2-3
...
void findBackgroundFraction_upperFitRange(const char* collisionSystem = "pAu")
{
  ...
  cout << setprecision(3);
  cout << " $p_T$ [GeV/c] & West Arm & East Arm & Both Arms \\\\ " 
       << FIT_RANGE_MIN << " to " << FIT_RANGE_MAX << endl;
  for( int b = 0; b < NUM_PT_BINS; b++ )
  ...
  TH1D *rWestHist = new TH1D("rEast",";r;",NUM_PT_BINS, PT_BINS);
  TH1D *rEastHist = new TH1D("rWest",";r;",NUM_PT_BINS, PT_BINS);    
  TH1D *rHist = new TH1D("rSqrt",";r;",NUM_PT_BINS, PT_BINS);
  for (int i=0; i<NUM_PT_BINS; ++i)
  { 
    rWestHist->SetBinContent(i+1,r[0][i]); 
    rEastHist->SetBinContent(i+1,r[1][i]);         
    rHist->SetBinContent(i+1,rSqrt[i]);
  }
  rWestHist->Write();
  rEastHist->Write();
  rHist->Write();
  ...
  //TGraph *rGraph = new TGraph(NUM_PT_BINS, PT_BIN_CENTERS, rSqrt);
  //rGraph->Draw();
