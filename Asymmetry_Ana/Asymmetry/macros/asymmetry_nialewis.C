...
void asymmetry_nialewis( const char* particle = "dp",
		...
		const bool isolated = false )//both must be set to false for dp
{
   if( particle != "dp" && particle != "pi0" && particle != "eta" )
    cout << "Error! particle can only be dp, pi0, or eta!  "
  ...
  if( isolated )
    cout << "*****Pair isolation cut! ";
  else
    cout << "*****No pair isolation cut";

  if( dpBackground )
    cout << " and evaluating " << particle 
	 << " asymmetry as a function of photon pt*****" << endl;
  else
    cout << " and evaluating " << particle
	 << " asymmetry as a function of particle pt*****" << endl;
  cout << endl;

  TString inputDataFileName = "/direct/phenix+u/workarea/nialewis/Run15ppPhotons/Asymmetry/macros/curated_";
  inputDataFileName += particle;
  if( dpBackground )
    inputDataFileName += "_dp_background";
  ...
  */
  ...
  if( dpBackground )
    dataTree->SetBranchAddress( "pt1Bin", &pt1Bin );
  else
    dataTree->SetBranchAddress( "ptBin",  &ptBin );
  if( isolated )
    {
      dataTree->SetBranchAddress( "energy1",     &energy1 );
      dataTree->SetBranchAddress( "energy2",     &energy2 );
      dataTree->SetBranchAddress( "coneEnergy1", &coneEnergy1 );
    }
  ...
  for( int i = 0; i < numEntries; i++ )
    {
      dataTree->GetEntry(i);
      if( i%5000000 == 0 ) cout << "Processed " << i << " entries " << endl;
      ...
      if( pt1 > 5 && pt1 < 18 && (energy1 * 0.1 > (coneEnergy1 - energy2)) )
	{
      ...
      */
      //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
      if( particle == "dp" && ptBin >= 0  )//direct photon regular old asymmetry
	{
	       ...
	       && pt1Bin >= 0 )
	{
	  ...
	  //cout << "Entry " << i << " pt1bin " << pt1Bin << endl;
	       ...
	       && ptBin >= 0 && (energy1 * 0.1 > (coneEnergy1 - energy2) ) )
	{
	  asymmetry.incrementCounts( fillBin, ptBin, arm, spinPattern );
	  accCorr.increment( arm, ptBin, px1 + px2, py1 + py2 );
	  //cout << "Entry " << i << " ptbin " << ptBin << endl;
	}
      /**/
