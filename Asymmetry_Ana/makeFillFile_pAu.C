...
const int NUM_FILLS = 78;  // This corresponds to the number of fills used in the analysis (after bad runs are removed) -- 144 for the p+p pi0 and eta TSSA analysis  -- currently 79 for the pAu analysis based on Norbert's good run list and additional QA
const int NUM_FILLS_IN_FILE = 91;  // This corresponds to the number of lines in polarization_pAu.txt 

// NOTE: Including a cut to ensure the polarization is non zero from the spin DB removes 8 runs (83 -> 75) -- I should find the proper polarization values from the CNI group and see if these runs are accounted for. Also be sure to exclude runs where they say the spin pattern is not right!!

void makeFillFile_pAu()
{
  ifstream textFile;
  ...
  textFile.open( "polarization_pAu.txt" );
  ...
  TFile *polFile = TFile::Open("../pAu_goodruns_ana664.root");
  ...
  TFile *outFile = new TFile( "fill_pAu.root", "RECREATE");
  float relLumB;
  ...
  for( int i = 0; i < numEntries; i++ )
    {
      ...
      // For some reason there is no data in the pair_tree corresponding to this fill, so possibly there is a mistake in the taxi code? Or every candidate in the fill did not pass the cuts...
      if (fillNumber==19110) {continue;}
      
      // this is just a test //
      //if (fillNumber==19114 || fillNumber==19120) {continue;}
